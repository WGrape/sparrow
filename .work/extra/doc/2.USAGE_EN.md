...
### (3) Common Configuration Adjustments

#### (1) Changing Namespace (CONTAINER_NAMESPACE)
...
If you don't know which port number to use, you add your service(such as ```langchain```) to the ```SUPPORT_SERVICE_LIST``` variable in the ```/.work/config/.env.amd64``` or ```/.work/config/.env.arm64``` file. Then run this command bellow.
...
#### (3) Add Your Service to SUPPORT_SERVICE_LIST and ENABLE_SERVICE_LIST in Your /.work/config/.env File

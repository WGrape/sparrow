...
4. helper.sh runs upcompose:
     ...
     - Compare with the stored hash in  at project root.
     - If unchanged, skip regeneration (manual edits to root docker-compose.yml are preserved).
     - If changed, regenerate: copy .work/config/docker-compose.yml to root, append each service compose (remove duplicated services: header), then save new hash to .
...
## 1. Image Types and Relationship
...
## 3. The Mounting of Service on The Host

...
4. helper.sh runs upcompose:
     - Compute an md5 hash of all inputs: root .env + .work/config/docker-compose.yml + each enabled service's {service}/docker-compose.yml. Uses `md5sum` (Linux) or `md5` (macOS); if neither is available, hash check is skipped and regeneration always runs.
     - Compare with the stored hash in .upcompose_hash at project root.
     - If unchanged, skip regeneration (manual edits to root docker-compose.yml are preserved).
     - If changed, regenerate: copy .work/config/docker-compose.yml to root, append each service compose (remove duplicated services header), then save new hash to .upcompose_hash.
...
flowchart TD
    ...
    H --> I
    I --> J[Run upcompose]
    J --> JH{Inputs hash changed?}
    JH -- No --> JN[Skip regeneration: keep existing docker-compose.yml]
    JH -- Yes --> K[Copy .work/config/docker-compose.yml to root]
    K --> L[Append each enabled service docker-compose.yml]
    L --> LS[Save new hash to .upcompose_hash]
    JN & LS --> M[Generated root .env + docker-compose.yml]
...
4. Root docker-compose.yml is regenerated only when inputs change (hash-based check on root .env + .work/config/docker-compose.yml + service compose files). The hash is computed via `md5sum` (Linux) or `md5` (macOS); if neither command is available, the hash check is skipped and regeneration always runs. Manual edits to root docker-compose.yml persist as long as no input file changes. The hash is stored in `.upcompose_hash` at project root.
...
## 2. sparrowtool Toolkit

The sparrowtool provides advanced features such as image uploading and new service creation, and is a companion toolkit for sparrow.

### (1) How to Upload Image

#### (a) Upload a Basic Image
...
You can use the ```sparrowtool``` to create new service. If you want to create a ```jupyter``` service, you can use this command below:
...
##### (i) Assign a Port Number

If you don't know which port number to use, you add your service(such as ```langchain```) to the ```SUPPORT_SERVICE_LIST``` variable in the ```/.work/config/.env.amd64``` or ```/.work/config/.env.arm64``` file. Then run this command below:
...
##### (ii) Modify Your Service Configuration

You must modify these image variables in ```./jupyter/.env``` file as below:
...
##### (iii) Add Your Service to SUPPORT_SERVICE_LIST and ENABLE_SERVICE_LIST in Your /.work/config/.env File

The ```SUPPORT_SERVICE_LIST``` and ```ENABLE_SERVICE_LIST``` variables are defined in ```/.work/config/.env.amd64``` or ```/.work/config/.env.arm64``` file, you should add ```jupyter``` the new service to both variables.
...
Now, you can use the following command to start the service:

```bash
./sparrow startone jupyter
```

## 3. Image Types and Relationship
...
2. Keep IMAGE_APP_{SERVICE}_VERSION as latest for local development unless you need explicit version pinning.
3. Push images with ./sparrowtool upload when you need team or CI/CD reuse.
4. Never skip the `FROM_PLATFORM` variable. Omitting it from pull, build, or compose will cause Docker to silently use the host's native architecture, leading to architecture mismatches when images are shared across amd64 and arm64 machines.
...
- Projects based on environments such as Python/PHP/Go will be mounted to their ```data``` directories.

## 6. Service Lifecycle Hooks

### (1) What are hooks

Each service can drop up to 4 scripts into its `hooks/` directory; sparrow runs each at a fixed point in the start/stop flow:

| Script | When it runs |
|---|---|
| `before_start.sh` | **Before** the container starts (before pull/build and `docker compose up`) |
| `after_start.sh` | **After** the container starts |
| `before_stop.sh` | **Before** the container stops |
| `after_stop.sh` | **After** the container stops |

Typical uses: generate or seed config files the container needs in `before_start.sh`; clean up scratch resources in `after_stop.sh`.

The dispatcher lives in the `sparrow` script: it looks for `{service}/hooks/{hook_type}.sh` and executes it if present. A missing file is treated as a no-op (not an error).

### (2) Hooks are sourced into the current shell — **do not use `exit`**

⚠️ **Important footgun**: hooks are not forked into a subshell. Sparrow runs them with `.` (source) inside its own main shell. The non-obvious consequence:

- Writing `exit 0` / `exit 1` in a hook **terminates the entire sparrow process**. The remaining steps (pull, build, `docker compose up`) are silently skipped, and the user sees what looks like a clean exit — very easy to misdiagnose.
- To return early from a hook, use **`return 0` / `return 1`** instead. That only exits the sourced block; sparrow's main flow keeps going.
...
# Config already initialized — skip and let sparrow continue.
if [ -f "./myservice/data/config.json" ]; then
    print_info "config already exists, skip init"
    return 0          # ← correct: exits the hook only
fi

# ... initialization logic ...
if ! some_init_command; then
    print_error "init failed"
    return 1          # ← still use return, even on the failure path
fi
```

By the same logic, **do not** use `set -e` or anything like `kill $$` inside a hook — they affect the parent shell. If you need to signal a precondition failure, log it with `print_error` and `return 1`.

### (3) What is available inside a hook

`sdk.sh` has already been sourced by the time your hook runs, so you can use sparrow's helpers directly:

- `print_info` / `print_warn` / `print_error` — colored log helpers
- All exported variables from `.env` (including `ENABLE_SERVICE_LIST` and per-service `IMAGE_*_VERSION`)
- `${SPARROW_BASE_PATH}` — project root

Re-sourcing the sdk inside a hook (as defensive guard) is safe.

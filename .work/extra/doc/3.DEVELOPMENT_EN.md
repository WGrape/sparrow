...
4. helper.sh runs upcompose:
     - Compute an md5 hash of all inputs: root .env + .work/config/docker-compose.yml + each enabled service's {service}/docker-compose.yml.
...
4. Root docker-compose.yml is regenerated only when inputs change (hash-based check on root .env + .work/config/docker-compose.yml + service compose files). Manual edits to root docker-compose.yml persist as long as no input file changes. The hash is stored in  at project root.

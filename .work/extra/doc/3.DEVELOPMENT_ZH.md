...
4. helper.sh 执行 upcompose：
	 - 复制 .work/config/docker-compose.yml 到根目录 docker-compose.yml。
	 - 追加每个启用服务的 {service}/docker-compose.yml（去掉重复的 services: 头）。
...
flowchart TD
	...
	H --> I
	I --> J[执行 upcompose]
	J --> K[复制 .work/config/docker-compose.yml 到根目录]
	K --> L[追加每个启用服务的 docker-compose.yml]
	L --> M[生成根目录 .env 与 docker-compose.yml]
	M --> N[启动或更新服务容器]
```
...
4. 常规开发中应将根目录 docker-compose.yml 视为生成产物，真实配置来源是 .work/config 与各服务配置文件。

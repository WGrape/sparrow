...
4. helper.sh 执行 upcompose：
	 - 计算所有输入文件的 md5 哈希：根目录 .env + .work/config/docker-compose.yml + 每个启用服务的 {service}/docker-compose.yml。
...
4. 根目录 docker-compose.yml 仅在输入文件变化时才重新生成（基于哈希比对：根目录 .env + .work/config/docker-compose.yml + 各服务 compose 文件）。只要输入未变化，对根目录 docker-compose.yml 的手动修改会被保留。哈希值存储在项目根目录的 `.upcompose_hash` 文件中。
...
- `./sparrow status` 输出当前 `CONTAINER_NAMESPACE` 值，便于确认当前激活的环境。

<div align="center"> <h1>Q&A Document</h1> </div>

## 1. Do Services Such As Redis and Mysql Support Cluster Mode?

### (1) Answer
No

### (2) Explain
Perhaps it will be supported in the future, but not at the moment because the use case is not very common.

## 2. Install and Start Docker/Docker-Compose

> Docker Release Notes: https://docs.docker.com/release-notes/

### (1) Install

#### a. Linux

If you find that accessing ```gitHub.com``` is slow or failed, you can try to switch a mirror source.

```bash
# 1. install docker
curl -fsSL https://get.docker.com | bash -s docker --mirror Aliyun

# 2. install docker-compose by using curl.
# if it's very slow, you'd better use the next solution.
curl -L https://github.com/docker/compose/releases/download/1.27.2/docker-compose-`uname -s`-`uname -m` > /usr/local/bin/docker-compose
chmod +x /usr/local/bin/docker-compose

# 3. install docker-compose by using pip3
pip3 install --upgrade pip
pip3 install docker-compose

# 4. install docker-compose by using yum
# pay attention to using at least version 1.20 or higher. Using a lower version may result in a failed compose.
yum install -y docker-compose
```

#### b. Mac

```Docker Desktop``` Installation of mac is here : https://docs.docker.com/desktop/release-notes/#4242 (you can choose any version).

### (2) Start

Check the Docker status by running the command ```docker info```. If it is not running, you need to start Docker first.

<img width="500" src="https://github.com/WGrape/sparrow/assets/35942268/22c72dca-c62a-43d5-81a1-deb4fca749bd" >

```bash
# Check Docker Status.
sudo systemctl status docker

# Start Docker.
sudo systemctl start docker
```

## 3. Some Common Errors

### (1) failed programming external connectivity … iptables: No chain/target/match by that name”

> failed programming external connectivity … iptables: No chain/target/match by that name

```bash
service docker restart
iptables -L 
```

### (2) docker-compose failed

> Compose failed, and the prompt for the docker-compose command appeared when do compose. #5

The reason for the error is that your docker-compose version is too low. It is recommended to upgrade your docker-compose version. [Issues#5](https://github.com/WGrape/sparrow/issues/5)

```bash
# upgrade to 1.27.2
curl -L https://github.com/docker/compose/releases/download/1.27.2/docker-compose-`uname -s`-`uname -m` > /usr/local/bin/docker-compose
chmod +x /usr/local/bin/docker-compose
```

<img width="500" src="https://github.com/WGrape/sparrow/assets/35942268/89fdae1c-8d71-4e2f-8c81-0b61686947ff" >

## 4. Architecture Mismatch: compose re-pulls and fails with "pull access denied"

### (1) Symptom

pull succeeds and a local tag `sparrow-app-{service}:latest` exists, but `docker compose up` reports:

```
pull access denied for sparrow-app-{service}, repository does not exist or may require 'docker login'
```

### (2) Root Cause

1. `docker pull` was called **without** `--platform`, so Docker pulled the image matching the host's native architecture (e.g. amd64 on an Intel/amd64 machine).
2. The local tag `sparrow-app-{service}:latest` now holds an **amd64** image.
3. `docker compose up` reads `platform: linux/arm64` in the service definition and detects that the local image architecture does not match.
4. Compose automatically attempts to re-pull the image to get the correct architecture.
5. `sparrow-app-{service}` is not a real registry repository, so the pull is denied — **error**.

### (3) Fix

`docker pull` must always include `--platform ${FROM_PLATFORM}`. This is already enforced in `dockerhub.sh`. If you call `docker pull` manually, always pass `--platform` explicitly:

```bash
docker pull --platform linux/arm64 lvsid/sparrow-app-mysql:latest
```

## 5. Docker Build Warnings: InvalidDefaultArgInFrom and SecretsUsedInArgOrEnv

### (1) Symptom

During `docker build`, the following warnings appear:

```
WARN: InvalidDefaultArgInFrom: Default value for ARG ${IMAGE_OFFICIAL_REDIS_NAME}:${IMAGE_OFFICIAL_REDIS_VERSION}
      results in empty or invalid base image name (line 4)

WARN: InvalidDefaultArgInFrom: Default value for ARG sparrow-basic-redis:${IMAGE_BASIC_REDIS_VERSION}
      results in empty or invalid base image name (line 3)

WARN: SecretsUsedInArgOrEnv: Do not use ARG or ENV instructions for sensitive data (ARG "REDIS_PASSWORD")

WARN: SecretsUsedInArgOrEnv: Do not use ARG or ENV instructions for sensitive data (ARG "MYSQL_ROOT_PASSWORD")

WARN: LegacyKeyValueFormat: "ENV key=value" should be used instead of legacy "ENV key value" format
```

<img width="1246" height="420" alt="Image" src="https://github.com/user-attachments/assets/c90b2941-01dd-4294-aab4-590ade2f4a65" />

### (2) Explanation

These are **BuildKit linter warnings only — they do not affect the build result**.

- **`InvalidDefaultArgInFrom`**: `ARG` declarations before `FROM` have no default value. BuildKit's static analysis checks the `FROM` line before `--build-arg` values are injected, sees empty strings, and warns that the image name looks invalid. At actual build time the real values are passed in and everything works correctly.
- **`SecretsUsedInArgOrEnv`**: BuildKit detects that a variable name contains keywords like `PASSWORD` or `SECRET` and recommends using `--secret` instead of `ARG`/`ENV` for sensitive data. This is a security best-practice suggestion and does not break anything.
- **`LegacyKeyValueFormat`**: The Dockerfile uses the old `ENV key value` syntax instead of the modern `ENV key=value` syntax. Both forms work identically; this is a style warning from BuildKit encouraging the newer format.

### (3) Resolution

Safely ignore these warnings. The build and container runtime are not affected.

## 6. Container Stuck in "Starting" During docker compose up

### (1) Symptom

```
+++++ stage10: do compose... +++++
[+] up 0/1
 ⠼ Container sparrow_container_test_mysql Starting   17.5s
```

The container stays in `Starting` state indefinitely and never becomes healthy.

### (2) Causes

There are two common causes:

- **Network/proxy issue**: Docker Compose checks the image digest against the registry when starting a container. If the proxy blocks outbound connections to `registry-1.docker.io`, this request hangs and the container appears stuck. This is the most common cause.
- **Dirty data directory after architecture switch**: When switching from amd64 to arm64 (or vice versa), the service's `data/` directory may contain files written by the previous architecture. On first start the new container performs crash recovery, which can take several minutes and look like a hang.

### (3) Fix

**For network/proxy issues** (most common):

`docker compose up` is called with `--pull never` in Sparrow, which skips the registry digest check. If you still see the hang, check Docker Desktop proxy settings (Settings → Resources → Proxies) and ensure `registry-1.docker.io` is reachable, or temporarily disable the proxy.

**For dirty data directory after architecture switch**:

Clear the service's data directory and restart:

```bash
# Example for mysql
docker stop sparrow_container_test_mysql
rm -rf ./mysql/data/*
./sparrow startone mysql

# Example for redis
docker stop sparrow_container_test_redis
rm -f ./redis/data/dump.rdb
./sparrow startone redis
```

Only do this if the data in that directory is not needed.

### (3) how to find the ip of container

You should use the ```docker inspect``` command to find the ip of container.

```bash
docker inspect sparrow_container_test_go
```

<img width="317" alt="image" src="https://github.com/WGrape/sparrow/assets/35942268/7661e020-03cc-4b4d-8937-d2f57b62e4ee">

### (4) mongodb authentication failed

If you've tried various methods and still encounter authentication failures, then delete your local data mount directory, for example, delete the directory ```sparrow/mongodb/data```.

- https://stackoverflow.com/questions/59274734/where-is-mongodb-config-file-located-when-running-not-as-an-ubuntu-service
- https://stackoverflow.com/questions/34559557/how-to-enable-authentication-on-mongodb-through-docker
- https://stackoverflow.com/questions/4881208/how-to-secure-mongodb-with-username-and-password
- https://stackoverflow.com/questions/37372684/mongodb-3-2-authentication-failed
- https://stackoverflow.com/questions/60394290/mongo-db-docker-image-authentication-failed

### (5) ssdb compose failed

<img width="700" alt="截屏2024-03-08 00 09 36" src="https://github.com/WGrape/sparrow/assets/35942268/ebaaba07-277d-4fe9-b82d-f116498cd53c">

Maybe you missed the following steps.

- delete ```/.env``` file
- must add ```ssdb``` service to ```ENABLE_SERVICE_LIST``` in th ```/.work/config/.env.amd64{arm64}``` file.

You must follow the steps of [How to Create New Service](https://github.com/WGrape/sparrow/blob/main/.work/extra/doc/3.DEVELOPMENT_EN.md#2-how-to-create-new-service),  youcan't skip any step.

### (5) ssdb build error with './var' is not a directory or not exists!

<img src="https://github.com/WGrape/sparrow/assets/35942268/4f1cd94e-1a29-4752-ba96-30e80cde04f5" width="500">

Firstly, search the error message on the ssdb project of github.

<img src="https://github.com/WGrape/sparrow/assets/35942268/50418b68-b710-4b2f-a955-5eb0d643946b" width="500">

Now, you will find the error from ```app_args.work_dir```, so search ```work_dir``` int the conf file.

<img src="https://github.com/WGrape/sparrow/assets/35942268/d2e1a8c3-2ad0-456f-b207-475d7cdd0420" width="500">

Finally, you already find the reason, just go to midify the ```work_dir``` to correct path.

## 7. Typical scenarios

- Add a new service:
    1. Generate from .work/extra/service_example via sparrowtool.
    2. Add service to ENABLE_SERVICE_LIST.
    3. Rerun Sparrow to regenerate root config and start service.
- Change global network/system settings:
    1. Update .work/config/.env.* templates.
    2. Regenerate and validate via Sparrow commands.

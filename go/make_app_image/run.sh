...
# bring the entire service directory into the build context.
cd $service_name/
if ! docker build \
  --build-arg FROM_PLATFORM=${FROM_PLATFORM} \
  --build-arg IMAGE_BASIC_GO_VERSION=${IMAGE_BASIC_GO_VERSION} \
  \
  --build-arg GO_CONTAINER_PORT=${GO_CONTAINER_PORT} \
  \
  -f ./make_app_image/Dockerfile \
  -t sparrow-app-$service_name:${IMAGE_APP_GO_VERSION} . ; then

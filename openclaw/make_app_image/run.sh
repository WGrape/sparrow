...
print_stage "try to pull/build sparrow-basic-${service_name} $IMAGE_BASIC_OPENCLAW_VERSION"
if pull basic $service_name $IMAGE_BASIC_OPENCLAW_VERSION; then
...
if ! docker build \
  ...
  --build-arg IMAGE_BASIC_OPENCLAW_VERSION=${IMAGE_BASIC_OPENCLAW_VERSION} \
  \
  -f ./make_app_image/Dockerfile \
  -t sparrow-app-$service_name:${IMAGE_APP_OPENCLAW_VERSION} . ; then

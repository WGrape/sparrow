...
print_stage "try to pull/build sparrow-basic-${service_name} $IMAGE_BASIC_SSDB_VERSION"
if pull basic $service_name $IMAGE_BASIC_SSDB_VERSION; then
...
if ! docker build \
  ...
  --build-arg IMAGE_BASIC_SSDB_VERSION=${IMAGE_BASIC_SSDB_VERSION} \
  \
  -f ./make_app_image/Dockerfile \
  -t sparrow-app-$service_name:${IMAGE_APP_SSDB_VERSION} . ; then
